    ...
    <div class="col-lg-12 ch-heading">
        <div class="row">
            ...
            <div class="pull-right">
				<?php echo $this->Html->link('<i class="glyphicon glyphicon-envelope"></i> Messages', array('controller' => 'messages', 'action' => 'listmessages', 'superadmin' => true), array('escape' => false, 'class' => 'btn btn-lmd btn-success', 'title' => 'Messages', 'id' => 'listMessages')); ?>
                ...
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo $this->Paginator->sort('user_name', 'Username/Email'); ?></th>
						...
						if (isset($data) && !empty($data)) {
							foreach ($data as $message_data) {
								...
								<tr>
									...
									<td><?php echo ((isset($message_data['Receiver']['user_name'])) ? ($message_data['Receiver']['user_name'] . '@' . Configure::read('SITE_EMAIL.Email'))  : ''); ?></td>
									<td>
									...
									<td class="td-actions">
										...
										<?php echo $this->Html->link('<i class="glyphicon glyphicon-envelope"></i> ', array('controller' => 'messages', 'action' => 'view_sent_message', base64_encode($message_data['Message']['id'])), array('escape' => false, 'class' => 'btn btn-xs btn-info viewRow', 'title' => 'View Message Detail')); ?>
										<!-- <a href="#" class="btn btn-xs btn-warning" title="Admin Profile"><i class="glyphicon glyphicon-user"></i> </a> -->
										<?php // echo $this->Html->link('<i class="glyphicon glyphicon-warning-sign"></i> ', 'javascript:void(0)', array('data-name' => $message_data['Receiver']['id'],'escape' => false, 'class' => 'btn btn-xs btn-success sendNotificationRow', 'title' => 'Send Notification', 'data-href' => $this->HTML->url(array('controller' => 'notifications', 'action' => 'send_notification', base64_encode($message_data['Receiver']['id']))))); ?>
										<?php echo $this->Html->link('<i class="glyphicon glyphicon-trash"></i> ', 'javascript:void(0)', array('data-name' => $message_data['Message']['subject'], 'escape' => false, 'class' => 'btn btn-xs btn-danger rowDelete', 'title' => 'Delete Message', 'data-href' => $this->Html->url(array('controller' => 'messages', 'action' => 'delete_sent_message', base64_encode($message_data['UserMessage']['id']))))); ?>
									</td>
								</tr>
								<?php
							}
...
<?php echo $this->Element('message/superadmin_view_sent_message'); ?>
<!-- Modal Send Message-->
<?php echo $this->Element('message/superadmin_compose_message'); ?>
<!-- Modal Send Message End -->
<!-- Modal Reply Message -->
<?php // echo $this->Element('message/superadmin_reply_message'); ?>
<!-- Modal Reply Message End -->
<!-- Modal Send Notification -->
<?php echo $this->Element('notifications/superadmin_send_notification'); ?>
<!-- Modal Send Notification -->
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		//$("#sendNotificationRow").click(function(event) {
			//event.preventDefault();
			// $('#SendNotificationModel > div.modal-dialog > sendNotificationForm').attr('href', $(this).attr('data-href'));
			// $('#SendNotificationModel > div.modal-dialog > div.modal-content > div.modal-body > center > #UserNotificationReciverId').val($(this).attr('data-name'));
