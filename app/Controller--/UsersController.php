...
class UsersController extends AppController {
	...
	public function login_unlock() {
		...
		if($this->Cookie->check('UserData')) {
			...
			$UserData = $this->User->findByUserName(
				$this->Cookie->read('UserData.user_name'), 
				array('user_name', 'first_name', 'last_name', 'business_name', 'display_name', 'full_name', 'profile_photo')
			);
	...
	public function logout() {
		if($this->Session->check('User')) {
			...
			$this->UserNotification->updateAll(
				array('UserNotification.is_current_dismiss' => 0),
				array(
		...
		if (!empty($this->request->data['value'])) {
			...
			if(!empty($found)) {
				$this->AdminUser->updateAll(
					array('AdminUser.admin_id' => $this->request->data['value']),
					array('AdminUser.user_id' => $userData['User']['id'])
				);
			} else {
	...
	public function updateToutStatus() {
		$this->autoRender = false;
		$this->layout = 'ajax';
		...
		if(!empty($userId)) {
			$this->User->id = $userId;
                        if($this->User->saveField('is_tour_completed', 1, false)){return TRUE;}else{return FALSE;}
		}
	}
}
