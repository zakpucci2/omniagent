        ...
        if (!empty($this->request->data['value'])) {
            ...
            if ($this->SupportTicket->save($data['SupportTicket'])) {
                ...
                if (!empty($this->request->data['value'])) {
                    ...
                    //pr($user_data);
                    $curecnt_ticket = $this->SupportTicket->read(array("SupportTicket.user_id"));
                    ...
                    $tic_id=$this->request->data['pk'];
                    $user_noti = array(
                        0 => array("Notification" => array(
                                "subject" => $sub,
                                "body" => "Your ticket #$tic_id has been assigned to $admin_name",
                        ...
                        1 => array("Notification" => array(
                                "subject" => $sub,
                                "body" => "Ticket #$tic_id has been assigned to you",
                                "is_push_notification" => 0
                            ),
                            "UserNotification" => array(
                                "sender_id" => $currentUserSession["User"]["id"],
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->SupportTicket->validates()) {
                ...
                if ($this->SupportTicket->save($this->request->data)) {
                    ...
                    $this->loadModel("UserNotification");
                    $tic_id=$this->SupportTicket->getLastInsertId();
                    $user_noti = array(
                        0 => array("Notification" => array(
                                "subject" => $sub,
                                "body" => "A new ticket #$tic_id has been created by $admin_name",
                                "is_push_notification" => 0
                            ),
                            "UserNotification" => array(
    ...
    public function superadmin_mark_complete($id = null) {
        ...
        if ($this->SupportTicket->exists()) {
            ...
            if ($this->SupportTicket->saveField('closed_date', date(Configure::read('DB_SAVE_DATETIME_FORMAT.DateTime'), time()))) {
                ...
                $currentUserSession = $this->Session->read('User');
                //pr($user_data);
                $curecnt_ticket = $this->SupportTicket->read(array("SupportTicket.user_id", "SupportTicket.admin_id"));

                //$admin_name = sprintf("%s %s", $user_data["User"]["first_name"], $user_data["User"]["last_name"]);
                $sub = sprintf("Close: Ticket #%s", $id);
                $msg_ticket = sprintf("Ticket #%s has been closed", $id);
