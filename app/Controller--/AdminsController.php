...
class AdminsController extends AppController {
	...
	public function admin_listregularadmins() {
		...
		$my_data = $this->AdminTeam->find("list", array("joins" => array(array("table" => "admin_release_requests", "alias" => "AdminReleaseRequest", "type" => "INNER", "conditions" => array("AdminTeam.user_id=AdminReleaseRequest.user_id", "AdminTeam.admin_id=AdminReleaseRequest.admin_id"))), "fields" => "AdminTeam.user_id"));




		if ($this->request->isGet() && isset($_GET['search'])) {
			$condition = array('User.user_type_id' => Configure::read('UserType.admin'), 'User.is_trusted_admin' => 0, 'User.is_deleted !=' => 1, "NOT" => array("User.id" => array_values($currentTeamMembers), "User.id" => array_values($my_data)), 'OR' => array('User.user_name LIKE' => "{$_GET['search']}%", 'User.first_name LIKE' => "{$_GET['search']}%", 'User.last_name LIKE' => "{$_GET['search']}%", 'User.email LIKE' => "{$_GET['search']}%"));
		} else {
			$condition = array('User.user_type_id' => Configure::read('UserType.admin'), 'User.is_trusted_admin' => 0, 'User.is_deleted !=' => 1, "NOT" => array("User.id" => array_values($currentTeamMembers), "User.id" => array_values($my_data)));
		}
	...
	public function acceptRejectReleaseRequest() {
		...
		if ($this->request->is('post')) {
			...
			if (
				$this->AdminReleaseRequest->saveField('is_request_accepted', $this->request->data['requestStatus'], array('validate' => false)) && $this->AdminReleaseRequest->saveField('days_admin_approved', $this->request->data['requestApprovedValue'], array('validate' => false)) && $this->AdminReleaseRequest->saveField('approved_date', date("Y-m-d H:i:s"), array('validate' => false))
			) {
