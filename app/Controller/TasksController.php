...
class TasksController extends AppController {
    ...
    public function admin_listtasks() {
        ...
        $tasksList = $this->Task->find('list');
        $this->set(compact('data', 'tasksList'));
    }
    ...
    public function view_task($id = null) {
        ...
        if ($this->RequestHandler->isAjax()) {
            ...
            $tastData = $this->TaskHistory->find("all", array('conditions' => array('TaskHistory.task_id' => $task['UserTask']['id']), 'order' => 'TaskHistory.created asc'));
            if (!empty($tastData)) {
    ...
    public function admin_view_task($id = null) {
        ...
        if ($this->RequestHandler->isAjax()) {
            ...
            $tastData = $this->TaskHistory->find("all", array('conditions' => array('TaskHistory.user_task_id' => $taskData['UserTask']['id']), 'order' => 'TaskHistory.created asc'));
            if (!empty($tastData)) {
                foreach ($tastData as $k => &$val) {
                    $tastData[$k] = $val;
                }
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Task->validates()) {
                ...
                if ($this->Task->save($this->request->data['Task'], false)) {
                    ...
                    $notificationData['Notification']['notification_type_id'] = 16;
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->UserTask->validates()) {
                ...
                if ($this->UserTask->save($this->request->data['UserTask'], false)) {
                    ...
                    $this->loadModel('Notification');
                    $this->loadModel('UserNotification');
                    $notificationData['Notification']['subject'] = "New task " . $currTask['UserTask']['task_title'] . " has been assigned to you.";
                    $notificationData['Notification']['body'] = "Hey " . ($currTask['Client']['first_name'] . " " . $currTask['Client']['last_name']) . "! " . ($currTask['Admin']['first_name'] . " " . $currTask['Admin']['last_name']) . "  is hard at work on your task; " . ($currTask['UserTask']['task_title']) . "! Check in with your Web Agent for further details.";
                    $notificationData['Notification']['notification_type_id'] = 4;
                    ...
                    $this->Session->setFlash('Task has been assigned to client successfully.', 'default', 'success');
                ...
                if ($this->UserTask->save($this->request->data, false, array('task_title', 'deadline_datetime', 'status_completed', 'task_description', 'is_completed'))) {
                    ...
                    if ($this->TaskHistory->save($save_arr)) {
                        ...
                        if ($this->request->data['UserTask']['status_completed'] == 25 || $this->request->data['UserTask']['status_completed'] == 50 || $this->request->data['UserTask']['status_completed'] == 100) {
                            ...
                            $notificationData['Notification']['notification_type_id'] = 6;
                            $savedNotification = $this->Notification->save($notificationData);
                            if (!empty($savedNotification)) {
                                $userNotificationData['UserNotification']['notification_id'] = $this->Notification->id;
                                $userNotificationData['UserNotification']['sender_id'] = $this->Session->read('User.User.id');
    ...
    public function admin_edittask_his($id = null) {
        $currentUserSession = $this->Session->read('User');
        $this->loadModel('TaskHistory');
        $PopupTitle = "Edit task history";
        $this->set("PopupTitle", $PopupTitle);
        if ($this->request->is('post')) {
            $this->request->data['TaskHistory']['id'] = $id;
            if ($this->TaskHistory->save($this->request->data)) {
                $this->Session->setFlash('Task has been updated successfully', 'default', 'success');
                $this->redirect(array('controller' => 'tasks', 'action' => 'listtasks', 'admin' => true));

                $taskData = $this->TaskHistory->find("first", array('conditions' => array('TaskHistory.id' => $id),
                    "fields" => array("TaskHistory.*,User.*,Task.name"),
                    'joins' => array(
                        array('table' => 'users',
                            'alias' => 'User',
                            'type' => 'INNER',
                            'conditions' => array('TaskHistory.user_task_id = User.id')
                        ),
                        array('table' => 'tasks',
                            'alias' => 'Task',
                            'type' => 'INNER',
                            'conditions' => array('TaskHistory.task_id = Task.id')
                        ),
                    )
                ));
            ...
            //echo $this->TaskHistory->getLastQuery();
            //pr($taskData);
            $this->request->data = $taskData;
    ...
    public function admin_ajax_task_his_update($id = null) {
        $this->autoRender = false;
        if ($this->request->is('post')) {
            $this->request->data['TaskHistory']['id'] = $id;
            if ($this->TaskHistory->save($this->request->data)) {
                $taskData = $this->TaskHistory->find("first", array('conditions' => array('TaskHistory.id' => $id),
                    "fields" => array("TaskHistory.*")
                ));
                return json_encode($taskData);
                exit;
            }
    ...
    public function admin_deltask_his($id = NULL) {
        $this->autoRender = FALSE;
        $this->TaskHistory->id = $id;
        if ($this->TaskHistory->exists()) {
            if ($this->TaskHistory->delete()) {
                return $id;
                exit;
            }
        }
    ...
    public function admin_complete_task($id = null) {
        ...
        if ($this->UserTask->exists()) {
            ...
            $notificationData['Notification']['notification_type_id'] = 5;
            $savedNotification = $this->Notification->save($notificationData);
            if (!empty($savedNotification)) {
                $this->request->data['UserNotification']['notification_id'] = $this->Notification->id;
