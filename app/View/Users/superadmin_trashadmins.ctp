...
<div><br>
    <div class="col-lg-12 ch-heading">
        <div class="row">
            <div class="pull-left"><strong class="lead"><i class="glyphicon glyphicon-trash"></i> Trash Administrators</strong></div>
			<div class="pull-right">
				<?php echo $this->Html->link('<i class="glyphicon glyphicon-globe"></i> Administrators', array('controller' => 'users', 'action' => 'listadmins', 'superadmin' => true), array('escape' => false, 'class' => 'btn btn-lmd btn-success', 'title' => 'List Administrators')); ?>
			</div>
        </div>
    </div>
    <br>
						...
						if (isset($data) && !empty($data)) {
							foreach ($data as $clientdata) {
								...
								<tr>
									...
									<td><?php echo $this->Html->link('RESTORE', 'javascript:void(0)', array('data-name' => $clientdata['User']['first_name'] . ' ' . $clientdata['User']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-primary act_go', 'title' => 'Restore Admin', 'data-href' => $this->Html->url(array('controller' => 'users', 'action' => 'restoreadmin', base64_encode($clientdata['User']['id']))))); ?></td> 
								</tr>
								<?php
							}
						} else {
