        ...
        <div class="box-content" style="border-radius:10px;">
            <div class="todo metro">
                <ul class="todo-list" style="border-radius:10px;">
                    <?php //echo '<pre>'; print_r($taskData); die; ?>
                    <?php $notodo = 0; $class = 'green';
                        foreach($taskData as $todo): 
                            if(!empty($todo['Task']['info1']) || !empty($todo['Task']['info2']) || !empty($todo['Task']['info3']) || !empty($todo['Task']['info4']) || !empty($todo['Task']['info5'])){ ?>
                                <li style="padding:5px 0px; color: #646464 !important;">
                                    <i class="icon-todo-check green"></i>                               
                                    <?php echo $todo['UserTask']['task_title']; ?>
                                </li>
                        <?php }
                            ?>
                        <?php if(isset($todo['Task']['info1']) && !empty($todo['Task']['info1'])): 
                            $task1_val = $todo['UserTask']['task1_completed'];
                            $notodo = 1;
                            ?>
                            <li class="<?php echo ($class=='green') ? $class='red' : $class= 'green'; ?>" style="border-radius:10px;<?php echo (isset($task1_val) && $task1_val=='1') ? 'text-decoration:line-through;' : ''; ?>">
                                <a class="action <?php echo (isset($task1_val) && $task1_val=='1') ? 'icon-check' : 'icon-check-empty'; ?>" href="#" rel="<?php echo $todo['UserTask']['id'].'-1'; ?>"></a>	
                                <?php echo $todo['Task']['info1']; ?>
                                <strong><?php echo date('m/d/Y',strtotime($todo['Task']['created'])); ?></strong>
                            </li>
                        <?php endif; ?>
                        <?php if(isset($todo['Task']['info2']) && !empty($todo['Task']['info2'])): 
                            $task2_val = $todo['UserTask']['task2_completed'];
                            $notodo = 1;
                            ?>
                            <li class="<?php echo ($class=='green') ? $class='red' : $class= 'green'; ?>" style="border-radius:10px;<?php echo (isset($task2_val) && $task2_val=='1') ? 'text-decoration:line-through;' : ''; ?>">
                                <a class="action <?php echo (isset($task2_val) && $task2_val=='1') ? 'icon-check' : 'icon-check-empty'; ?>" href="#" rel="<?php echo $todo['UserTask']['id'].'-2'; ?>"></a>	
                                <?php echo $todo['Task']['info2']; ?>
                                <strong><?php echo date('m/d/Y',strtotime($todo['Task']['created'])); ?></strong>
                            </li>
                        <?php endif; ?>
                        <?php if(isset($todo['Task']['info3']) && !empty($todo['Task']['info3'])): 
                            $task3_val = $todo['UserTask']['task3_completed'];
                            $notodo = 1;
                            ?>
                            <li class="<?php echo ($class=='green') ? $class='red' : $class= 'green'; ?>" style="border-radius:10px;<?php echo (isset($task3_val) && $task3_val=='1') ? 'text-decoration:line-through;' : ''; ?>">
                                <a class="action <?php echo (isset($task3_val) && $task3_val=='1') ? 'icon-check' : 'icon-check-empty'; ?>" href="#" rel="<?php echo $todo['UserTask']['id'].'-3'; ?>"></a>	
                                <?php echo $todo['Task']['info3']; ?>
                                <strong><?php echo date('m/d/Y',strtotime($todo['Task']['created'])); ?></strong>
                            </li>
                        <?php endif; ?>
                        <?php if(isset($todo['Task']['info4']) && !empty($todo['Task']['info4'])): 
                            $task4_val = $todo['UserTask']['task4_completed'];
                            $notodo = 1;
                            ?>
                            <li class="<?php echo ($class=='green') ? $class='red' : $class= 'green'; ?>" style="border-radius:10px;<?php echo (isset($task4_val) && $task4_val=='1') ? 'text-decoration:line-through;' : ''; ?>">
                                <a class="action <?php echo (isset($task4_val) && $task4_val=='1') ? 'icon-check' : 'icon-check-empty'; ?>" href="#" rel="<?php echo $todo['UserTask']['id'].'-4'; ?>"></a>	
                                <?php echo $todo['Task']['info4']; ?>
                                <strong><?php echo date('m/d/Y',strtotime($todo['Task']['created'])); ?></strong>
                            </li>
                        <?php endif; ?>
                        <?php if(isset($todo['Task']['info5']) && !empty($todo['Task']['info5'])): 
                            $task5_val = $todo['UserTask']['task5_completed'];
                            $notodo = 1;
                            ?>
                            <li class="<?php echo ($class=='green') ? $class='red' : $class= 'green'; ?>" style="border-radius:10px;<?php echo (isset($task5_val) && $task5_val=='1') ? 'text-decoration:line-through;' : ''; ?>">
                                <a class="action <?php echo (isset($task5_val) && $task5_val=='1') ? 'icon-check' : 'icon-check-empty'; ?>" href="#" rel="<?php echo $todo['UserTask']['id'].'-5'; ?>"></a>	
                                File uploaded
                                <strong><?php echo date('m/d/Y',strtotime($todo['Task']['created'])); ?></strong>
                            </li>
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <?php if($notodo == 0){ ?>
                        <li class="red">No To Do Assigned</li>
                    <?php } ?>
            ...
            [
                ...
                },
                {
                    ...
                    onShow: function(tour) {
                        ...
                        $("#userNameActionLi").addClass("open");
...
if (!empty($usersession)) {
    // $this->Common->end_tour_now($usersession);
}