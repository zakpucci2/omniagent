    ...
    <div class="col-lg-12 ch-heading">
        <div class="row">
            ...
            <div class="pull-right">
				<?php echo $this->Html->link('<i class="glyphicon glyphicon-floppy-saved"></i> Closed Tickets', array('controller' => 'support_tickets', 'action' => 'closedtickets', 'superadmin' => true), array('escape' => false, 'class' => 'btn btn-lmd btn-success', 'title' => 'Closed Tickets', 'id' => 'closedTickets')); ?>
            </div>
        </div>
    </div>
    <br><br>
                ...
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo $this->Paginator->sort('SupportTicket.admin_id', 'Assigned To'); ?></th>
							...
							foreach ($data as $supportticket_data) {
								...
								<tr>
									...
									<?php
										$admin = '';
										$adminId = '';
											if (!empty($supportticket_data['SupportTicket'])) {
												$admin = $supportticket_data['Admin']['first_name'] . " " . $supportticket_data['Admin']['last_name'];
												$adminId = $supportticket_data['Admin']['id'];
												?>
												<a href="javascript: void(0);" id="<?php echo $supportticket_data['SupportTicket']['id']; ?>" data-type="select" data-value="<?php echo @adminId; ?>" data-name="<?php echo $supportticket_data['SupportTicket']['id']; ?>" data-pk="<?php echo $supportticket_data['SupportTicket']['id']; ?>" data-url="<?php echo $this->Html->url(array('controller' => 'support_tickets', 'action' => 'assignAdmin', 'superadmin' => false)); ?>" data-original-title="Assign Admin"><?php echo @$admin; ?></a>
												<?php
											} else {
												?>
												<a href="javascript: void(0);" id="<?php echo $supportticket_data['SupportTicket']['id']; ?>" data-type="select" data-name="<?php echo $supportticket_data['SupportTicket']['id']; ?>" data-pk="<?php echo $supportticket_data['SupportTicket']['id']; ?>" data-url="<?php echo $this->Html->url(array('controller' => 'support_tickets', 'action' => 'assignAdmin', 'superadmin' => false)); ?>" data-original-title="Assign Admin">Assign Admin</a>
												<?php
											}
										?>
									</td>
									...
									<td class="td-actions">
										<?php echo $this->Html->link('<i class="glyphicon glyphicon-eye-open"></i>', array('controller' => 'support_tickets', 'action' => 'view_ticket', base64_encode($supportticket_data['SupportTicket']['id']), 'superadmin' => true), array('escape' => false, 'class' => 'btn btn-xs btn-default viewTicketRow', 'title' => 'View Support Ticket Detail')); ?>
										<?php echo $this->Html->link('<i class="glyphicon glyphicon-share-alt"></i>', array('controller' => 'support_tickets', 'action' => 'reply_ticket', base64_encode($supportticket_data['SupportTicket']['id']), 'superadmin' => true), array('escape' => false, 'class' => 'btn btn-xs btn-primary replyTicketRow', 'title' => 'Reply To Sender')); ?>
										<!-- <a href="#" class="btn btn-xs btn-warning" alt="User Profile" title="User Profile"><i class="glyphicon glyphicon-user"></i> </a> -->
										<?php 
									
										
										if ($supportticket_data['SupportTicket']['status'] != 5 && $supportticket_data["SupportTicket"]["admin_id"]!='' && $supportticket_data["SupportTicket"]["admin_id"]!=NULL  ) {
											echo $this->Html->link('<i class="glyphicon glyphicon-ok"></i>', 'javascript:void(0)', array('data-name' => $supportticket_data['SupportTicket']['subject'], 'escape' => false, 'class' => 'btn btn-xs btn-success markComplete', 'title' => 'Mark Complete', 'data-href' => $this->Html->url(array('controller' => 'support_tickets', 'action' => 'mark_complete', base64_encode($supportticket_data['SupportTicket']['id'])))));
										} ?>
								...
								<script type="text/javascript">
									$(document).ready(function() {
										$('#<?php echo $supportticket_data['SupportTicket']['id']; ?>').editable({
...
<!-- Modal View Ticket -->
<?php echo $this->Element('supporttickets/superadmin_view_ticket'); ?>
<!-- Modal View Message-->
<!-- Modal Reply Message-->
<?php echo $this->Element('supporttickets/superadmin_reply_ticket'); ?>
<!-- Modal Reply Message-->
<!-- Modal Delete -->
...
<div id="MarkCompleteModel" tabindex="-1" class="modal fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <center>
                    Are you sure want to close this support ticket  "<strong>selected user</strong>"?
                </center>	
            </div>
            <div class="modal-footer">
                <center>
