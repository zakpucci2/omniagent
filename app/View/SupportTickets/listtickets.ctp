		...
		<tbody>
			...
			if (isset($data) && !empty($data)) {
				...
				foreach ($data as $ticket_data) {
					...
					<?php echo $this->Html->link('<i class="halflings-icon white trash"></i> Delete', 'javascript:void(0)', array('data-name' => $ticket_data['SupportTicket']['subject'], 'escape' => false, 'class' => 'btn btn-danger btn-md delRow', 'title' => 'Delete SupportTicket', 'data-href' => $this->Html->url(array('controller' => 'support_tickets', 'action' => 'delete_ticket', base64_encode($ticket_data['SupportTicket']['id']))))); ?>
						</td>
					</tr>
					<?php
					$counter++;
