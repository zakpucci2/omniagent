...
<div><br>
    <div class="col-lg-12 ch-heading">
        <div class="row">
            <div class="pull-left"><strong class="lead"><i class="glyphicon glyphicon-ok"></i> Notifications</strong></div>
						...
						if (isset($data) && !empty($data)) {
							foreach ($data as $notification_data) {
								...
								<tr>
									...
									<td scope="col"><?php
										if (!empty($notification_data['UserNotification'])) {
											...
											foreach ($notification_data['UserNotification'] as $user_notification_data) {
												$receivers .= (isset($user_notification_data['Receiver']['first_name']) ? $user_notification_data['Receiver']['first_name'] : '') . ' ' . (isset($user_notification_data['Receiver']['last_name']) ? $user_notification_data['Receiver']['last_name'] : '') . ',';
									...
									<td scope="col">
										<?php echo $this->Html->link('<i class="glyphicon glyphicon-search"></i> ', array('controller' => 'notifications', 'action' => 'view_notification', base64_encode($notification_data['Notification']['id']), 'superadmin' => true), array('escape' => false, 'class' => 'btn btn-sm btn-default viewNotificationRow', 'title' => 'View Notification Detail')); ?>									
...
<!-- Modal Add Client -->
<?php echo $this->Element('super_admin/addNotification'); ?>
<?php echo $this->Element('notifications/superadmin_view_notification'); ?>
<?php echo $this->Element('notifications/superadmin_send_push_notification'); ?>
<!-- Modal Edit Client -->
<script type="text/javascript">
	$(document).ready(function() {
